...
function generateReasons(product: Product, selectedTags: string[]): string[] {
  ...
  if (selectedTags.includes('bodycam') && productTags.includes('bodycam')) {
    reasons.push(`Integrates a <strong>1080p Bodycam with IR night vision</strong> and GPS live tracking for legal evidence.`);
  }

  if (reasons.length < 2) {
    reasons.push(`Highlights its <strong>${product.comparison.protection}</strong> and <strong>${product.comparison.batteryRuntime}</strong>.`);
  }
    ...
    <section id="finder" class="py-10 sm:py-16 bg-navy-800 relative overflow-hidden scroll-mt-20">
      ...
      <div class="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-6 sm:mb-10 relative z-10">
          <span class="text-crimson-400 font-bold tracking-widest uppercase text-sm mb-2 block">Tactical Mission Assistant</span>
          <h2 class="text-2xl sm:text-3xl md:text-4xl font-extrabold text-white">FIND YOUR EXACT SYSTEM</h2>
          <p class="text-navy-300 text-sm sm:text-base mt-1">4 direct questions. Operational profile analysis. Technically justified recommendation.</p>
        </div>
    ...
    <div class="bg-navy-900/80 backdrop-blur-sm border border-navy-700 rounded-xl p-3 sm:p-6 shadow-2xl animate-fade-in">
      <div class="mb-4">
        <div class="flex items-center justify-between text-xs sm:text-sm mb-2">
          <span class="text-crimson-400 font-bold uppercase tracking-wider">Step ${step + 1} of ${QUIZ_QUESTIONS.length}</span>
      ...
      <div class="mb-4">
        <h3 class="text-base sm:text-xl font-bold text-white mb-1">${question.title}</h3>
        <p class="text-navy-300 text-xs sm:text-sm">${question.subtitle}</p>
      </div>
        ...
        ${question.options.map((opt, optionIndex) => `
          <button data-option-index="${optionIndex}" data-tags="${opt.tags.join(',')}" class="finder-option w-full text-left p-3 sm:p-4 rounded-xl border-2 border-navy-700 bg-navy-900/50 hover:border-crimson-600 hover:bg-navy-800/50 transition-all duration-200 group flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="font-semibold text-white text-xs sm:text-sm group-hover:text-crimson-200 transition-colors leading-snug">${opt.label}</div>
              <div class="text-navy-400 text-[11px] sm:text-sm mt-0.5 group-hover:text-navy-300 transition-colors hidden sm:block">${opt.description}</div>
            </div>
  ...
  if (!primary) {
    return `
      <div class="bg-navy-900/80 backdrop-blur-sm border border-navy-700 rounded-xl p-4 sm:p-8 shadow-2xl text-center animate-fade-in">
        ...
        <h3 class="text-lg sm:text-xl font-bold text-white mb-2">No exact matches found</h3>
        <p class="text-navy-300 text-sm mb-6">Adjust your answers or contact an expert for personalized guidance.</p>
        <button onclick="resetFinder()" class="px-5 py-3 rounded-full font-bold uppercase tracking-wider text-white bg-crimson-700 hover:bg-crimson-800 transition-colors text-sm">Restart Test</button>
      </div>
    `;
  }
    ...
    <div class="bg-navy-900/80 backdrop-blur-sm border border-navy-700 rounded-xl p-3 sm:p-6 shadow-2xl animate-fade-in">
      <div class="flex items-center justify-between mb-3 gap-2">
        <div class="inline-flex items-center gap-2 bg-crimson-900/50 border border-crimson-700 rounded-full px-3 py-1.5">
          ...
          <span class="text-white font-bold text-xs sm:text-sm">${matchScore}% Mission Match</span>
        </div>
      ...
      <div class="flex items-center gap-3 mb-3">
        <img src="${product.image}" alt="${product.name}" class="w-20 h-20 sm:w-28 sm:h-28 rounded-xl bg-navy-900 border border-navy-700 object-contain p-1 flex-shrink-0">
        <div class="min-w-0">
          <h3 class="text-white font-bold text-sm sm:text-base mb-1">${product.name}</h3>
          <span class="text-crimson-400 font-bold uppercase tracking-wider text-[10px] sm:text-xs">${product.badge}</span>
          <p class="text-navy-400 text-[11px] sm:text-sm mt-1 line-clamp-2">${product.description}</p>
        </div>
      </div>
      ...
      ${alternative ? `
        <div class="bg-navy-900/30 border border-navy-700 rounded-xl p-2.5 mb-3">
          <p class="text-navy-400 text-[10px] sm:text-xs mb-1.5 font-medium uppercase tracking-wider">Solid Alternative</p>
          <div class="flex items-center gap-2">
            <div class="w-12 h-12 sm:w-14 sm:h-14 bg-navy-900 rounded-lg overflow-hidden border border-navy-700 flex-shrink-0">
              <img src="${alternative.image}" alt="${alternative.name}" class="w-full h-full object-contain p-0.5">
            </div>
            <div class="flex-1 min-w-0">
              <p class="text-white font-semibold text-xs sm:text-sm truncate">${alternative.name}</p>
              <p class="text-crimson-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider">${alternative.badge}</p>
            </div>
          </div>
        </div>
      ` : ''}
